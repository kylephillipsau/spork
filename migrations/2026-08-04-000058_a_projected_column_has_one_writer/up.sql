-- Migration 58: a projected column has one writer, and a function says when it last changed.
--
-- D104, settling question 169. Structural only: no fold logic changes. Bodies below are
-- the live definitions with one comment line added, because rewriting a fold from the
-- migration that introduced it is how migration 56 nearly reverted D65 and D68.
--
-- Two holes:
--
--   1. S43 checks that a registered family writes the column it claims. Nothing checks
--      that nobody else writes it too. D101 was two maintainers of the same family
--      owning one column between them for fifty-two migrations, values always right.
--
--   2. A CREATE OR REPLACE carries no record of what it replaced. The decision record
--      names the migration that introduced a function, not the one that last changed
--      it, so the obvious place to look is reliably the wrong one.
--
-- The marker is a body comment, not a COMMENT ON FUNCTION, so pasting an older body
-- keeps the older marker and an intentional edit bumps it:
--
--     -- last changed: migration NNN (Dxxx)
--
-- S55 (exclusive writers, with a declared multi-writer allowlist for the D101 pair)
-- and S56 (every VOLATILE projection function carries the marker) live in the suite.
